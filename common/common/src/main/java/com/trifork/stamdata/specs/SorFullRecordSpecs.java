...
public class SorFullRecordSpecs {
	...
	public static final RecordSpecification INSTITUTION_OWNER = RecordSpecification.createSpecification("SORInstitutionOwner", "sorIdentifier",
				...
				field("fkEanLocationCode", 10).doAllowNull(),
				field("fkPostalAddressInformation", 10).numerical(),
				field("fkVirtualAddressInfomation", 10).numerical(),
				field("fkSorStatus", 10).numerical()
			);
    ...
    public static final RecordSpecification HEALTH_INSTITUTION_RECORD_TYPE = RecordSpecification.createSpecification("SORHealthInstitution", "SorIdentifier",
    			// Den første komponent kan blive lige så lang som der er cifre i antallet af enheder:  
    			//  SOR-kode der relaterer til enheden. Genereres automatisk. En SOR-kode består af fire 
    			//  komponenter:
    			//   - Fortløbende nummer 
    			//   - Namespace (7 cifre: "1000016")
    			//   - Partition-ID (2 cifre)
    			//   - Checksum-ciffer (1 cifre).
    			//  Den første mulige SOR-kode er dermed '11000016002'.
	            field("sorIdentifier", 20).numerical(),
	            field("entityName", 60),
	            ...
	            field("geographicalParentRelation", 10).numerical().doAllowNull(),
