...
public class ViewServlet extends HttpServlet
{
    ...
    private static final Logger logger = getLogger(ViewServlet.class);
    ...
    @Inject
    ViewServlet(@Registry Map<String, Class<? extends View>> viewMap, Provider<SecurityManager> securityManager, Provider<RecordDao> recordDao, Provider<AtomFeedWriter> writers)
    {
        this.viewMap = checkNotNull(viewMap);
    ...
    private void writeResponse(HttpServletRequest request, HttpServletResponse response) throws IOException
    {
        if (!isLegalOffsetParam(request))
        {
            ...
            logger.warn("Invalid parameter. offset={}, client_id={}.", getOffsetParam(request), securityManager.get().getClientId(request));
            return;
        }
        ...
        if (!isLegalCountParam(request))
        {
            ...
            logger.warn("Invalid parameter. count={}, client_id={}.", getCountParam(request), securityManager.get().getClientId(request));
            return;
        }
    ...
    private void fetchAndWriteRecords(HttpServletRequest request, HttpServletResponse response) throws IOException
    {
        ...
        Class<? extends View> entityType = viewMap.get(getViewName(request));
