 ...
 * <li>Handles all aspects of deserialization and error handling with respect to
 * DGWS.</li>
 * <li>Issue authorization tokens to authorized clients using the security
 * manager.</li>
 * </ul>
 * 
	...
	static class RequestProcessor
	{
		...
		public Reply process(Request request)
		{
			try
			{
				...
				audit.log("client_cvr=%s, entity_uri=%s", cvr, requestBody.getView());
				...
				{
					audit.log("cvr=%s, request_view=%s, expires_at=%s, access_granted=true", cvr, viewName, card.getExpiryDate());
				...
				{
					audit.log("cvr=%s, request_view=%s, access_granted=false", cvr, viewName);
					
					reply = factory.createNewErrorReply(request, CLIENT_SIDE_ERROR, "Your are not authorized to clone the requested resource.");
				}
