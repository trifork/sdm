...
import com.trifork.stamdata.importer.jobs.sor.xml.OrganizationalUnitEntity;
...
public class SOREventHandler extends DefaultHandler
{
	...
	private void createDatasets(Date snapshotDate)
	{
		...
		dataSets.setApotekDS(new CompleteDataset<Apotek>(Apotek.class, snapshotDate, DateUtils.FOREVER));
		dataSets.setYderDS(new CompleteDataset<Yder>(Yder.class, snapshotDate, DateUtils.FOREVER));
		dataSets.setPraksisDS(new CompleteDataset<Praksis>(Praksis.class, snapshotDate, DateUtils.FOREVER));
		dataSets.setSygehusDS(new CompleteDataset<Sygehus>(Sygehus.class, snapshotDate, DateUtils.FOREVER));
		dataSets.setSygehusAfdelingDS(new CompleteDataset<SygehusAfdeling>(SygehusAfdeling.class, snapshotDate,
				DateUtils.FOREVER));
	}
	...
	public void endElement(String uri, String localName, String qName) throws SAXException
	{
		...
		if ("SnapshotDate".equals(qName))
		{
			...
			try
			{
				createDatasets(toDate(elementValue));
			}
				...
				if (hie.getInstitutionType() == 394761003L || hie.getInstitutionType() == 550671000005100L)
				{
					...
					for (OrganizationalUnitEntity oue : hie.getOrganizationalUnitEntities())
					{
						...
						if (!oue.getSons().isEmpty())
						{
							...
							new SAXException("Lægepraksis skal ikke have en level 2 OrganizationalUnitEntity. SORId="
			...
			if (method != null)
			{
				...
				try
				{
					...
					else if (param.isAssignableFrom(Long.class))
					{
						...
						{
							logger.error("Number format exception on property " + qName + ", method "
									+ object.getClass().getName() + "." + method.getName() + "(" + param.getName()
									+ ")", e);
							throw e;
						}
					...
					{
						logger.error("Unsupported datatype for property " + qName + ", datatype was "
								+ param.getClass().getName());
						throw new Exception("Unsupported datatype for property " + qName + ", datatype was "
								+ param.getClass().getName());
					}
				}
				catch (IllegalArgumentException e)
				{
					logger.error("Illegal argument exception on property " + qName + ", method "
							+ object.getClass().getName() + "." + method.getName() + "(" + param.getName() + ")", e);
					throw e;
				}
				catch (IllegalAccessException e)
				{
					logger.error("Illegal access exception on property " + qName + ", method "
							+ object.getClass().getName() + "." + method.getName() + "(" + param.getName() + ")", e);
					throw e;
				}
				catch (InvocationTargetException e)
				{
					logger.error("Invocation target exception on property " + qName + ", method "
							+ object.getClass().getName() + "." + method.getName() + "(" + param.getName() + ")", e);
					throw e;
				}
			}
	...
	public static Date toDate(String xmlDate) throws java.text.ParseException
	{
		...
		try
		{
			date = formatter.parse(xmlDate);
		}
		catch (java.text.ParseException e)
		{
			logger.error("Invalid date found in xml. Expected 'yyyy-MM-dd' found " + xmlDate, e);
			throw e;
		}
