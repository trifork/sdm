...
public class FileSpoolerSetup
{
	...
	{
		if (stableSeconds == 0) stableSeconds = Integer.parseInt(getConfig("stableSeconds", "" + DEFAULT_STABLE_SECONDS));
	...
	private void resolveImporterClass(String importerName)
	{
		...
		{
			logger.error("Configuration error. You need to configure an import class to spooler " + getName() + ". Set property " + getConfigEntry("importerClass") + " to the class path of the importer");
		}
		try
		{
		...
		{
			logger.error("Configuration error. The configured import class (" + importerName + " to spooler " + getName() + "could net be found. Set property " + getConfigEntry("importerClass") + " to a valid importer");
		}
		catch (ClassCastException e)
		{
			logger.error("Configuration error. The configured import class (" + importerName + " to spooler " + getName() + "didn't implement interface " + FileImporter.class.getName() + ". Set property " + getConfigEntry("importerClass") + " to a valid importer");
		}
