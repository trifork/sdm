...
public class JobSpoolerSetup
{
	...
	private void resolveImporterClass(String executorName)
	{
		...
		{
			logger.error("Configuration error. You need to configure a executer class to the job " + getName() + ". Set property " + getConfigEntry("jobExecutorClass") + " to the class path of the job executor");
		}
		try
		{
		...
		{
			logger.error("Configuration error. The configured job executor class (" + executorName + " could not be found. " + "Set property " + getConfigEntry("jobExecutorClass") + " to a valid job executor");
		}
		catch (ClassCastException e)
		{
			logger.error("Configuration error. The configured job executor class (" + executorName + " didn't implement interface " + Job.class.getName() + ". Set property " + getConfigEntry("jobExecutorClass") + " to a valid job executor");
		}
	}
