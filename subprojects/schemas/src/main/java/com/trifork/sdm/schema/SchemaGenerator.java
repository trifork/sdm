...
public class SchemaGenerator
{
	...
	private void generateSchemaVersion(Class<? extends Record> type, Entity typeAnnotation, int version) throws IOException, XMLStreamException, FactoryConfigurationError
	{
		...
		String schemaFilePath = format("%s/%sV%d.xsd", target, resourceName, version);
		...
		Map<String, Method> elements = findAllElementsForVersion(type, version);
		...
		// For forward compatibility we and an any.
		
		writer.writeEmptyElement("any");
		writer.writeAttribute("minOccurs", "0");
		writer.writeAttribute("maxOccurs", "unbounded");
	...
	private Map<String, Method> findAllElementsForVersion(Class<? extends Record> type, int version) throws InvalidClassException
	{
	...
	private static Map<String, Method> sortByOrder(Map<String, Method> map)
	{
		List<Map.Entry<String, Method>> list = new LinkedList<Map.Entry<String, Method>>(map.entrySet());
		...
		Collections.sort(list, new Comparator<Map.Entry<String, Method>>()
		{
			...
			{
				int order1 = EntityHelper.getOrder(o1);
				int order2 = EntityHelper.getOrder(o2);
		...
		for (Iterator<Map.Entry<String, Method>> it = list.iterator(); it.hasNext();)
		{
