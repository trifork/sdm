...
public class EntityHelper {
	...
	assert id != null : format("Entity %s does not have a @Id method.",
		entity.getSimpleName());

	return id;
    }

    public static Method getMethodForColumnName(
	    Class<? extends Record> entitySet, String name) {
    ...
    public static boolean isDateColumn(Class<? extends Record> entitySet,
	    String name) throws SecurityException, NoSuchMethodException {

	return getMethodForColumnName(entitySet, name).getReturnType().equals(
		Date.class);
    }

    public static boolean isLongColumn(Class<? extends Record> entitySet,
	    String name) throws SecurityException, NoSuchMethodException {

	return getMethodForColumnName(entitySet, name).getReturnType().equals(
		Long.class)
		|| getMethodForColumnName(entitySet, name).getReturnType()
			.equals(long.class);
    }

    public static boolean isBooleanColumn(Class<? extends Record> entitySet,
	    String name) throws SecurityException, NoSuchMethodException {
    ...
    public static boolean isIntegerColumn(Class<? extends Record> entitySet,
	    String name) throws SecurityException, NoSuchMethodException {
    ...
    public static boolean isStringColumn(Class<? extends Record> entitySet,
	    String name) throws SecurityException, NoSuchMethodException {
	...
	if (resourceTypes == null) {
	    ...
	    final String INCLUDE_PACKAGE = com.trifork.stamdata.util.Record.class
		    .getPackage().getName();
	    ...
	    final String EXCLUDE_PACKAGE = com.trifork.stamdata.registre.doseringsforslag.Drug.class
		    .getPackage().getName();

	    Reflections reflector = new Reflections(new ConfigurationBuilder()
		    .setUrls(
			    ClasspathHelper
				    .getUrlsForPackagePrefix(INCLUDE_PACKAGE))
		    .filterInputsBy(
			    new FilterBuilder().include(
				    FilterBuilder.prefix(INCLUDE_PACKAGE))
				    .exclude(
					    FilterBuilder
						    .prefix(EXCLUDE_PACKAGE)))
		    .setScanners(new TypeAnnotationsScanner()));

	    // Serve all entities by deferring their URLs and using their
	    // annotations.

	    Set<Class<?>> entities = reflector
